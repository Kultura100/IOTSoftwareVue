            ...
            <div v-if="!camera.length"
                ...
                <div class="flex w-full flex-row lg:w-3/12">
                    <div class="ml-4 self-center overflow-x-hidden">
                        <div class="w-full truncate text-xl font-extrabold leading-5 tracking-tight">{{ __('dashboard.camerapanel.empty')}}</div>
                    </div>
                </div>
            </div>

            <div class="grid sm:grid-cols-2 sm:grid-rows-2 gap-1 grid-cols-1 grid-rows-4">
                <div v-for="(cam, index) in camera" :key="cam.id"
                    class="px-2 pt-2 lg:pt-1 lg:pb-1 lg:w-full bg-opacity-75 rounded-md text-slate-800 transition duration-500 ease-in-out hover:-translate-y-1 hover:shadow-lg lg:p-1"
                    style="background: rgba(0,0,0,0.2);">
                    <div class="flex flex-wrap flex-col">
                        <div class="flex-1 w-full h-full" v-if="cam.protocol_id == 1">
                            <div v-show="cameras[index].isLoaded"
                                class="flex justify-center items-center sm:h-[31.5rem] h-96 bg-gray-300 rounded-lg animate-pulse dark:bg-gray-700">
                            ...
                            <object @load="Img(index)" class="h-[200px] lg:h-full lg:w-full mx-auto rounded-md"
                                :data="cam.get_protocol_name.name + '://' + cam.source + ':' + cam.port + '/videostream.cgi?user=' + cam.login + '&amp;pwd=' + cam.password"
                                type="image/jpeg"></object>
                        </div>
                        <div v-else class="flex w-full h-[32rem] overflow-hidden">
                            <div v-show="cameras[index].isLoaded"
                                class="flex justify-center items-center w-full  bg-gray-300 rounded-lg animate-pulse dark:bg-gray-700">
                                <svg class="w-12 h-12 text-gray-200 dark:text-gray-600"
                                    xmlns="http://www.w3.org/2000/svg" aria-hidden="true" fill="currentColor"
                                    viewBox="0 0 384 512">
                                    <path
...
export default {
    ...
    methods: {
        fullscreen() {
            ...
            this.$refs.canvas.requestFullscreen?.() || this.$refs.canvas.webkitRequestFullScreen?.() || this.$refs.canvas.mozRequestFullScreen?.();
        },
        Img(id) {
