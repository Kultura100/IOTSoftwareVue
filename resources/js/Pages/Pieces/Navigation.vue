...
<template>
    ...
    <nav class="px-4 flex justify-between bg-white h-16 border-b-2 navigation">
        ...
        <ul class="flex items-center">
            <li>
                <h1 class="lg:pl-0 text-white truncate hidden lg:block lg:max-w-full">{{ title }}</h1>
            </li>
        </ul>
    ...
    <nav v-if="$page.props.Uzytkownik" class="fixed bottom-0 w-full bg-white z-20 lg:hidden flex overflow-x-auto navigation h-16">
        ...
        <button v-on:click="Logout()" class="flex flex-col flex-grow items-center justify-center overflow-hidden whitespace-no-wrap transition-colors duration-100 ease-in-out hover:bg-red-200 focus:outline-none focus:text-orange-500">
