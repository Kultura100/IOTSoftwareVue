    ...
    <AppLayout :title="__('tasktable.routename')">
        ...
        <div class="py-4">
            <div class="flex flex-col gap-2 ml-2 mr-2 lg:flex-row">
                <div class="w-full">
                    <div class="mb-3 flex w-full transform cursor-pointer flex-row rounded-md bg-white bg-opacity-75 p-6 transition duration-500 ease-in-out hover:-translate-y-1 hover:shadow-lg text-slate-300 lg:p-4" style="background: rgba(0, 0, 0, 0.2);">
                        ...
                        <JetResponsiveNavLink aria-label="timetable" :customclass="'flex  w-full focus:text-orange-500'" :href="route('timetableadd')">
                ...
                <div class="w-full">
                    <div class="mb-3 flex w-full transform cursor-pointer flex-col rounded-md bg-white bg-opacity-75 p-6 transition duration-500 ease-in-out hover:-translate-y-1 hover:shadow-lg text-slate-300 lg:flex-row lg:p-4 mx-auto" style="background: rgba(0, 0, 0, 0.2);">
                        <div class="flex w-full flex-row lg:w-full">
                            ...
                            <div class="w-full lg:ml-4 ml-1 self-center overflow-x-hidden">
                                <div v-if="item.type == 'alldata'" class="w-full truncate text-lg font-bold leading-6 tracking-tight">
                                    ({{ item.device.name }}) - {{ __('tasktable.types.full') }}
                                </div>
                                <div v-else-if="item.type == 'temperature'" class="w-full truncate text-lg font-bold leading-6 tracking-tight">
                                    ({{ item.device.name }}) - {{ __('tasktable.types.temperature') }}
                                </div>
                                <div v-else-if="item.type == 'humidity'" class="w-full truncate text-lg font-bold leading-6 tracking-tight">
                                    ({{ item.device.name }}) - {{ __('tasktable.types.humidity') }}
                                </div>
                                <div v-if="item.frequency_function == 'everyThirtyMinutes'" class="flex text-sm text-white font-bold break-words">
                                    {{ __('tasktable.types.frequency') }} <span class="mx-2 text-gray-300 leading-5">{{ __('tasktable.types.30min') }}</span>
                                </div>
                                <div v-else-if="item.frequency_function == 'hourly'" class="flex text-sm text-white font-bold break-words">
                                    {{ __('tasktable.types.frequency') }} <span class="mx-2 text-gray-300 leading-5">{{ __('tasktable.types.hour') }}</span>
                                </div>
                                <div v-else-if="item.frequency_function == 'everySixHours'" class="flex text-sm text-white font-bold break-words">
                                    {{ __('tasktable.types.frequency') }} <span class="mx-2 text-gray-300 leading-5">{{ __('tasktable.types.6hour') }}</span>
                                </div>
                                <div v-else-if="item.frequency_function == 'daily'" class="flex text-sm text-white font-bold break-words">
                                    {{ __('tasktable.types.frequency') }} <span class="mx-2 text-gray-300 leading-5">{{ __('tasktable.types.daily') }}</span>
                                </div>
                                <div v-else-if="item.frequency_function == 'weekly'" class="flex text-sm text-white font-bold break-words">
                                    {{ __('tasktable.types.frequency') }} <span class="mx-2 text-gray-300 leading-5">{{ __('tasktable.types.weekly') }}</span>
                                </div>
                                <div v-else-if="item.frequency_function == 'monthly'" class="flex text-sm text-white font-bold break-words">
                                    {{ __('tasktable.types.frequency') }} <span class="mx-2 text-gray-300 leading-5">{{ __('tasktable.types.monthly') }}</span>
                                </div>
                                <div v-else-if="item.frequency_function == 'weeklyOn'" class="flex text-sm text-white font-bold break-words">
                                ...
                                <div v-else-if="item.frequency_function == 'monthlyOn'" class="flex text-sm text-white font-bold break-words">
