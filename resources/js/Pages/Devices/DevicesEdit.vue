<template>
    ...
    <div class="flex flex-col items-center mx-2 lg:mx-auto rounded-md w-full lg:p-4 lg:w-1/3 mt-2 lg:mt-4" style="background: rgba(0,0,0,0.2);">
        ...
        <div>
            <JetButton @click="onSubmit">{{ __('dashboard.transmitterpanel.edit.buttontitle') }}</JetButton>
        </div>
    </div>
</AppLayout>
</template>
...
export default {
    ...
    mounted(){
        this.form.name = this.transmitter.name;
        this.form.transmitterId = this.transmitter.transmitterId;
        this.form.receiver_id = this.transmitter.receiver_id;
        this.form.id = this.transmitter.id;
    },
    methods: {
        onSubmit() {
            this.form.post('/updatetransmitter', {
                onSuccess: () => {
                    this.form.reset();
